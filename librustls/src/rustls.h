...
typedef enum rustls_handshake_kind
{
   ...
   * not yet unavailable, and the initial client hello was accepted by the
   * server.
   */
   ...
   *
   * The server can respond with a hello retry request (HRR) if the initial
   * client hello is unacceptable for several reasons, the most likely if no
   * supported key shares were offered by the client.
   */
 ...
 * that asynchronously, as opposed to
 * rustls_server_config_builder_set_hello_callback(), which doesn't work well
...
/**
 * A built client certificate verifier that can be provided to a
 * `rustls_server_config_builder` with
 ...
 *
 * A builder can be modified by, e.g.
 * `rustls_client_config_builder_load_roots_from_file`. Once you're done
 * configuring settings, call `rustls_client_config_builder_build` to turn it
 * into a *rustls_client_config.
 *
 ...
 *
 * `rustls_hpke` can be provided to `rustls_client_config_builder_enable_ech`
 * and `rustls_client_config_builder_enable_ech_grease()` to customize a
 ...
 * `rustls_root_cert_store_builder_add_pem`. Once you're done adding root
 * certificates, call `rustls_root_cert_store_builder_build` to turn it into a
 * `rustls_root_cert_store`. This object is not safe for concurrent mutation.
 */
typedef struct rustls_root_cert_store_builder rustls_root_cert_store_builder;

/**
 * A built server certificate verifier that can be provided to a
 * `rustls_client_config_builder` with
 ...
 *
 * Once you're done configuring settings, call
 * `rustls_web_pki_client_cert_verifier_builder_build` to turn it into a
 ...
 */
typedef struct rustls_web_pki_client_cert_verifier_builder
  rustls_web_pki_client_cert_verifier_builder;
 ...
 *
 * Once you're done configuring settings, call
 * `rustls_web_pki_server_cert_verifier_builder_build` to turn it into a
 * `rustls_server_cert_verifier`. This object is not safe for concurrent
 * mutation.
 *
 * See
 * <https://docs.rs/rustls/latest/rustls/client/struct.ServerCertVerifierBuilder.html>
 * for more information.
 */
typedef struct rustls_web_pki_server_cert_verifier_builder
 ...
 *
 * An implementation of this callback should attempt to read up to n bytes from
 * the network, storing them in `buf`. If any bytes were stored, the
 * implementation should set out_n to the number of bytes stored and return 0.
 *
 * If there was an error, the implementation should return a nonzero
 * rustls_io_result, which will be passed through to the caller.
 *
 * On POSIX systems, returning `errno` is convenient.
 *
 * On other systems, any appropriate error code works.
 *
 * It's best to make one read attempt to the network per call. Additional reads
 * will be triggered by subsequent calls to one of the `_read_tls` methods.
 *
 * `userdata` is set to the value provided to `rustls_connection_set_userdata`.
 * In most cases that should be a struct that contains, at a minimum, a file
 * descriptor.
 *
 * The buf and out_n pointers are borrowed and should not be retained across
 * calls.
 */
typedef rustls_io_result (*rustls_read_callback)(void *userdata, uint8_t *buf,
...
/**
 * A callback for `rustls_connection_write_tls` or
 * `rustls_accepted_alert_write_tls`.
 *
 * An implementation of this callback should attempt to write the `n` bytes in
 * buf to the network.
 *
 * If any bytes were written, the implementation should set `out_n` to the
 * number of bytes stored and return 0.
 *
 * If there was an error, the implementation should return a nonzero
 * `rustls_io_result`, which will be passed through to the caller.
 *
 ...
 * On other systems, any appropriate error code works.
 *
 * It's best to make one write attempt to the network per call. Additional
 * writes will be triggered by subsequent calls to rustls_connection_write_tls.
 *
 * `userdata` is set to the value provided to `rustls_connection_set_userdata`.
 * In most cases that should be a struct that contains, at a minimum, a file
 * descriptor.
 ...
 */
typedef uint32_t (*rustls_verify_server_cert_callback)(
 ...
 */
typedef void (*rustls_log_callback)(void *userdata,
                                    const struct rustls_log_params *params);
 ...
 *
 * If any bytes were written, the implementation should set out_n to the number
 * of bytes written and return 0.
 *
 * If there was an error, the implementation should return a nonzero
 * rustls_io_result, which will be passed through to the caller.
 *
 ...
 *
 * It's best to make one write attempt to the network per call. Additional
 * write will be triggered by subsequent calls to one of the `_write_tls`
 * methods.
 *
 * `userdata` is set to the value provided to `rustls_*_session_set_userdata`.
 * In most cases that should be a struct that contains, at a minimum, a file
 * descriptor.
 *
 * The iov and out_n pointers are borrowed and should not be retained across
 * calls.
 */
typedef rustls_io_result (*rustls_write_vectored_callback)(
 ...
 * client did not send this TLS extension, the default schemes in the rustls
 * library. See:
 ...
 * the call and may not be modified. Users of this API must copy any values
 * that they want to access when the callback returned.
 *
 * EXPERIMENTAL: this feature of rustls-ffi is likely to change in the future,
 * as the rustls library is re-evaluating their current approach to client
 * hello handling.
 */
 ...
 *
 * `hello` gives the value of the available client announcements, as
 * interpreted by rustls. See the definition of `rustls_client_hello` for
 * details.
 *
 * NOTE:
 * - the passed in `hello` and all its values are only available during the
 *   callback invocations.
 * - the passed callback function must be safe to call multiple times
 * concurrently with the same userdata, unless there is only a single config
 * and connection where it is installed.
 *
 * EXPERIMENTAL: this feature of rustls-ffi is likely to change in the future,
 * as the rustls library is re-evaluating their current approach to client
 * hello handling.
 ...
 *
 * `userdata` will be supplied based on
 * rustls_{client,server}_session_set_userdata.
 *
 ...
 */
typedef uint32_t (*rustls_session_store_get_callback)(
  rustls_session_store_userdata userdata, const struct rustls_slice_bytes *key,
  int remove_after, uint8_t *buf, size_t count, size_t *out_n);
 ...
 * must be implemented thread-safe.
 */
typedef uint32_t (*rustls_session_store_put_callback)(
  rustls_session_store_userdata userdata, const struct rustls_slice_bytes *key,
 ...
 *   *out_accepted.
 * - RUSTLS_RESULT_ACCEPTOR_NOT_READY: a full ClientHello has not yet been
 * read. Read more TLS bytes to continue.
 ...
 */
struct rustls_str rustls_accepted_server_name(
  const struct rustls_accepted *accepted);
 ...
 */
uint16_t rustls_accepted_signature_scheme(
  const struct rustls_accepted *accepted, size_t i);
 ...
 */
struct rustls_slice_bytes rustls_accepted_alpn(
  const struct rustls_accepted *accepted, size_t i);
 ...
 */
rustls_result rustls_accepted_into_connection(
  struct rustls_accepted *accepted, const struct rustls_server_config *config,
 ...
 * values from callback. See [`rustls_write_callback`] or [`AcceptedAlert`] for
 * more details.
 */
rustls_io_result rustls_accepted_alert_write_tls(
  struct rustls_accepted_alert *accepted_alert, rustls_write_callback callback,
  void *userdata, size_t *out_n);
 ...
 */
rustls_result rustls_certified_key_build(
  const uint8_t *cert_chain, size_t cert_chain_len, const uint8_t *private_key,
 ...
 * the `rustls_server_config` is freed.
 */
rustls_result rustls_certified_key_build_with_signing_key(
 ...
 *
 * 0 gives the end-entity certificate. 1 and higher give certificates from the
 * chain.
 *
 ...
 */
const struct rustls_certificate *rustls_certified_key_get_certificate(
  const struct rustls_certified_key *certified_key, size_t i);
 ...
 * by the application.
 */
rustls_result rustls_certified_key_clone_with_ocsp(
...
/**
 * Verify the consistency of this `rustls_certified_key`'s public and private
 * keys.
 *
 * This is done by performing a comparison of subject public key information
 * (SPKI) bytes between the certificate and private key.
 *
 * If the private key matches the certificate this function returns
 * `RUSTLS_RESULT_OK`, otherwise an error `rustls_result` is returned.
 */
rustls_result rustls_certified_key_keys_match(
  const struct rustls_certified_key *key);
 ...
 * extant certified_key may still hold an internal reference to the Rust
 * object.
 *
 ...
 * regardless of whether `rustls_root_cert_store_builder_build` was called.
 *
 * If you wish to abandon the builder without calling
 * `rustls_root_cert_store_builder_build`, it must be freed with
 * `rustls_root_cert_store_builder_free`.
 */
struct rustls_root_cert_store_builder *rustls_root_cert_store_builder_new(
  void);
 ...
 * certificates.
 */
rustls_result rustls_root_cert_store_builder_add_pem(
  struct rustls_root_cert_store_builder *builder, const uint8_t *pem,
 ...
 * certificates.
 */
rustls_result rustls_root_cert_store_builder_load_roots_from_file(
  struct rustls_root_cert_store_builder *builder, const char *filename,
  bool strict);
 ...
 *
 * The root cert store can be used in several
 * `rustls_web_pki_client_cert_verifier_builder_new` instances and must be
 ...
 */
rustls_result rustls_root_cert_store_builder_build(
  struct rustls_root_cert_store_builder *builder,
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 */
void rustls_root_cert_store_builder_free(
  struct rustls_root_cert_store_builder *builder);

/**
 * Free a rustls_root_cert_store previously returned from
 * rustls_root_cert_store_builder_build.
 *
 * Calling with NULL is fine. Must not be called twice with the same value.
 */
...
/**
 * Return a 16-bit unsigned integer corresponding to this cipher suite's
 * assignment from
 * <https://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml#tls-parameters-4>.
 *
 * The bytes from the assignment are interpreted in network order.
 */
uint16_t rustls_supported_ciphersuite_get_suite(
 ...
 * empty string. The lifetime of the `rustls_str` is the lifetime of the
 * program, it does not need to be freed.
 */
struct rustls_str rustls_supported_ciphersuite_get_name(
  const struct rustls_supported_ciphersuite *supported_ciphersuite);
 ...
 */
enum rustls_tls_version rustls_supported_ciphersuite_protocol_version(
  const struct rustls_supported_ciphersuite *supported_ciphersuite);

/**
 * Create a rustls_client_config_builder using the process default crypto
 * provider.
 *
 * Caller owns the memory and must eventually call
 * `rustls_client_config_builder_build`, then free the resulting
 * `rustls_client_config`.
 *
 * Alternatively, if an error occurs or, you don't wish to build a config,
 * call `rustls_client_config_builder_free` to free the builder directly.
 *
 * This uses the process default provider's values for the cipher suites and
 * key exchange groups, as well as safe defaults for protocol versions.
 *
 ...
 * call `rustls_client_config_builder_free` to free the builder directly.
 *
 * `tls_version` sets the TLS protocol versions to use when negotiating a TLS
 * session. `tls_version` is the version of the protocol, as defined in
 * rfc8446, ch. 4.2.1 and end of ch. 5.1. Some values are defined in
...
rustls_result rustls_client_config_builder_new_custom(
  const struct rustls_crypto_provider *provider, const uint16_t *tls_versions,
  size_t tls_versions_len, struct rustls_client_config_builder **builder_out);
 ...
 *
 * This increases the reference count of `verifier` and does not take
 * ownership.
 */
void rustls_client_config_builder_set_server_verifier(
 ...
 *
 * `protocols` must point to a buffer of `rustls_slice_bytes` (built by the
 * caller) with `len` elements.
 *
 ...
 */
rustls_result rustls_client_config_builder_set_alpn_protocols(
  struct rustls_client_config_builder *builder,
  const struct rustls_slice_bytes *protocols, size_t len);

/**
 * Enable or disable SNI.
 * <https://docs.rs/rustls/latest/rustls/struct.ClientConfig.html#structfield.enable_sni>
 */
void rustls_client_config_builder_set_enable_sni(
  struct rustls_client_config_builder *config, bool enable);
 ...
 */
rustls_result rustls_client_config_builder_set_certified_key(
  struct rustls_client_config_builder *builder,
  const struct rustls_certified_key *const *certified_keys,
  size_t certified_keys_len);

/**
 * Log key material to the file specified by the `SSLKEYLOGFILE` environment
 * variable.
 *
 * The key material will be logged in the NSS key log format,
 * <https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSS/Key_Log_Format>
 * and is compatible with tools like Wireshark.
 *
 * Secrets logged in this manner are **extremely sensitive** and can break the
 * security of past, present and future sessions.
 *
 * For more control over which secrets are logged, or to customize the format,
 * prefer `rustls_client_config_builder_set_key_log`.
 */
rustls_result rustls_client_config_builder_set_key_log_file(
  struct rustls_client_config_builder *builder);

/**
 * Provide callbacks to manage logging key material.
 *
 * The `log_cb` argument is mandatory and must not be `NULL` or a
 * `NullParameter` error is returned. The `log_cb` will be invoked with a
 * `client_random` to identify the relevant session, a `label` to identify the
 * purpose of the `secret`, and the `secret` itself. See the Rustls
 * documentation of the `KeyLog` trait for more information on possible labels:
 * <https://docs.rs/rustls/latest/rustls/trait.KeyLog.html#tymethod.log>
 *
 * The `will_log_cb` may be `NULL`, in which case all key material will be
 * provided to the `log_cb`. By providing a custom `will_log_cb` you may return
 * `0` for labels you don't wish to log, and non-zero for labels you _do_ wish
 * to log as a performance optimization.
 *
 * Both callbacks **must** be thread-safe. Arguments provided to the callback
 * live only for as long as the callback is executing and are not valid after
 * the callback returns. The callbacks must not retain references to the
 * provided data.
 *
 * Secrets provided to the `log_cb` are **extremely sensitive** and can break
 * the security of past, present and future sessions.
 *
 * See also `rustls_client_config_builder_set_key_log_file` for a simpler way
 * to log to a file specified by the `SSLKEYLOGFILE` environment variable.
 */
rustls_result rustls_client_config_builder_set_key_log(
  struct rustls_client_config_builder *builder,
 ...
 * have been retrieved from the DNS HTTPS record for the domain you intend to
 * connect to. This should be done using DNS-over-HTTPS to avoid leaking the
 * domain name you are connecting to ahead of the TLS handshake.
 *
 * At least one of the ECH configurations must be compatible with the provided
 * `rustls_hpke` instance. See `rustls_supported_hpke()` for more information.
 *
 ...
 *
 * The provided `ech_config_list_bytes` and `rustls_hpke` must not be NULL or
 * an error will be returned. The caller maintains ownership of the ECH config
 * list TLS bytes and `rustls_hpke` instance. This function does not retain any
 * reference to `ech_config_list_bytes`.
 *
 * A `RUSTLS_RESULT_BUILDER_INCOMPATIBLE_TLS_VERSIONS` error is returned if the
 * builder's TLS versions have been customized via
 * `rustls_client_config_builder_new_custom()` and the customization isn't
 * "only TLS 1.3". ECH may only be used with TLS 1.3.
 */
rustls_result rustls_client_config_builder_enable_ech(
  struct rustls_client_config_builder *builder,
  const uint8_t *ech_config_list_bytes, size_t ech_config_list_bytes_size,
  const struct rustls_hpke *hpke);

/**
 * Configure the client for GREASE Encrypted Client Hello (ECH).
 *
 * This is a feature to prevent ossification of the TLS handshake by acting as
 * though ECH were configured for an imaginary ECH config generated with one of
 * the `rustls_hpke` supported suites, chosen at random.
 *
 * The provided `rustls_client_config_builder` and `rustls_hpke` must not be
 * NULL or an error will be returned. The caller maintains ownership of both
 * the `rustls_client_config_builder` and the `rustls_hpke` instance.
 *
 * Calling this function will replace any existing ECH configuration set by
 * previous calls to `rustls_client_config_builder_enable_ech()` or
 * `rustls_client_config_builder_enable_ech_grease()`.
 ...
 * "only TLS 1.3". ECH may only be used with TLS 1.3.
 */
rustls_result rustls_client_config_builder_enable_ech_grease(
  struct rustls_client_config_builder *builder,
  const struct rustls_hpke *hpke);

/**
 * Turn a *rustls_client_config_builder (mutable) into a const
 * *rustls_client_config (read-only).
 */
rustls_result rustls_client_config_builder_build(
  struct rustls_client_config_builder *builder,
  const struct rustls_client_config **config_out);

/**
 * "Free" a client_config_builder without building it into a
 * rustls_client_config.
 *
 * Normally builders are built into rustls_client_config via
 * `rustls_client_config_builder_build` and may not be free'd or otherwise used
 * afterwards.
 *
 * Use free only when the building of a config has to be aborted before a
 * config was created.
 */
void rustls_client_config_builder_free(
  struct rustls_client_config_builder *config);

/**
 * Returns true if a `rustls_connection` created from the
 * `rustls_client_config` will operate in FIPS mode.
 *
 * This is different from `rustls_crypto_provider_fips` which is concerned
 * only with cryptography, whereas this also covers TLS-level configuration
 * that NIST recommends, as well as ECH HPKE suites if applicable.
 */
 ...
 *
 * Since `rustls_client_config` is actually an atomically reference-counted
 * pointer, extant client connections may still hold an internal reference to
 * the Rust object.
 *
 * However, C code must consider this pointer unusable after "free"ing it.
 *
 ...
 * the `rustls_connection` and must call `rustls_connection_free` when done
 * with it.
 *
 ...
 */
rustls_result rustls_client_connection_new(
  const struct rustls_client_config *config, const char *server_name,
  struct rustls_connection **conn_out);

/**
 * Set the userdata pointer associated with this connection. This will be
 * passed to any callbacks invoked by the connection, if you've set up
 * callbacks in the config. The pointed-to data must outlive the connection.
 */
void rustls_connection_set_userdata(struct rustls_connection *conn,
                                    void *userdata);

/**
 * Set the logging callback for this connection. The log callback will be
 * invoked with the userdata parameter previously set by
 * rustls_connection_set_userdata, or NULL if no userdata was set.
 */
void rustls_connection_set_log_callback(struct rustls_connection *conn,
                                        rustls_log_callback cb);

/**
 * Read some TLS bytes from the network into internal buffers. The actual
 * network I/O is performed by `callback`, which you provide. Rustls will
 * invoke your callback with a suitable buffer to store the read bytes into.
 * You don't have to fill it up, just fill with as many bytes as you get in one
 * syscall. The `userdata` parameter is passed through directly to `callback`.
 * Note that this is distinct from the `userdata` parameter set with
 * `rustls_connection_set_userdata`.
 * Returns 0 for success, or an errno value on error. Passes through return
 * values from callback. See rustls_read_callback for more details.
 ...
 * `rustls_connection_set_userdata`.
 * Returns 0 for success, or an errno value on error. Passes through return
 * values from callback. See rustls_write_callback for more details.
...
/**
 * Write all available TLS bytes to the network. The actual network I/O is
 * performed by `callback`, which you provide. Rustls will invoke your callback
 * with an array of rustls_slice_bytes, each containing a buffer with TLS bytes
 * to send. You don't have to write them all, just as many as you are willing.
 * The `userdata` parameter is passed through directly to `callback`. Note that
 * this is distinct from the `userdata` parameter set with
 * `rustls_connection_set_userdata`.
 * Returns 0 for success, or an errno value on error. Passes through return
 ...
 */
rustls_io_result rustls_connection_write_tls_vectored(
  struct rustls_connection *conn, rustls_write_vectored_callback callback,
  void *userdata, size_t *out_n);
 ...
 */
rustls_result rustls_connection_process_new_packets(
 ...
 *
 * Note: This may return `false` while there are still handshake packets
 * waiting to be extracted and transmitted with
 ...
 */
enum rustls_handshake_kind rustls_connection_handshake_kind(
  const struct rustls_connection *conn);
 ...
 *
 * Rustls internally manages key updates as required and so this function
 * should seldom be used. See the Rustls documentation for important caveats
 * and suggestions on occasions that merit its use.
 *
 * <https://docs.rs/rustls/latest/rustls/struct.ConnectionCommon.html#method.refresh_traffic_keys>
 */
rustls_result rustls_connection_refresh_traffic_keys(
  struct rustls_connection *conn);
 ...
 */
const struct rustls_certificate *rustls_connection_get_peer_certificate(
  const struct rustls_connection *conn, size_t i);
 ...
 */
uint16_t rustls_connection_get_protocol_version(
  const struct rustls_connection *conn);

/**
 * Retrieves the [IANA registered cipher suite identifier][IANA] agreed with
 * the peer.
 *
 * This returns `TLS_NULL_WITH_NULL_NULL` (0x0000) until the ciphersuite is
 * agreed.
 *
 * [IANA]:
 * <https://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml#tls-parameters-4>
 */
uint16_t rustls_connection_get_negotiated_ciphersuite(
  const struct rustls_connection *conn);
...
/**
 * Retrieves the [IANA registered supported group identifier][IANA] agreed with
 * the peer.
 *
 ...
 */
uint16_t rustls_connection_get_negotiated_key_exchange_group(
  const struct rustls_connection *conn);
 ...
 * need to be freed.
 */
struct rustls_str rustls_connection_get_negotiated_key_exchange_group_name(
  const struct rustls_connection *conn);
...
/**
 * Returns true if the `rustls_connection` was made with a
 * `rustls_client_config` or `rustls_server_config` that is FIPS compatible.
 *
 * This is different from `rustls_crypto_provider_fips` which is concerned
 * only with cryptography, whereas this also covers TLS-level configuration
 * that NIST recommends, as well as ECH HPKE suites if applicable.
...
/**
 * Constructs a new `rustls_crypto_provider_builder` using the process-wide
 * default crypto provider as the base crypto provider to be customized.
 *
 * When this function returns `rustls_result::Ok` a pointer to the
 * `rustls_crypto_provider_builder` is written to `builder_out`. It returns
 * `rustls_result::NoDefaultCryptoProvider` if no default provider has been
 * registered.
 *
 * The caller owns the returned `rustls_crypto_provider_builder` and must free
 * it using `rustls_crypto_provider_builder_free`.
 *
 * This function is typically used for customizing the default crypto provider
 * for specific connections. For example, a typical workflow might be to:
 *
 * * Either:
 *   * Use the default `aws-lc-rs` or `*ring*` provider that rustls-ffi is
 * built with based on the `CRYPTO_PROVIDER` build variable.
 *   * Call `rustls_crypto_provider_builder_new_with_base` with the desired
 * provider, and then install it as the process default with
 *     `rustls_crypto_provider_builder_build_as_default`.
 * * Afterward, as required for customization:
 *   * Use `rustls_crypto_provider_builder_new_from_default` to get a builder
 * backed by the default crypto provider.
 *   * Use `rustls_crypto_provider_builder_set_cipher_suites` to customize the
 * supported ciphersuites.
 *   * Use `rustls_crypto_provider_builder_build` to build a customized
 * provider.
 *   * Provide that customized provider to client or server configuration
 * builders.
 */
rustls_result rustls_crypto_provider_builder_new_from_default(
  struct rustls_crypto_provider_builder **builder_out);

/**
 * Constructs a new `rustls_crypto_provider_builder` using the given
 * `rustls_crypto_provider` as the base crypto provider to be customized.
 *
 * The caller owns the returned `rustls_crypto_provider_builder` and must free
 * it using `rustls_crypto_provider_builder_free`.
 *
 * This function can be used for setting the default process wide crypto
 * provider, or for constructing a custom crypto provider for a specific
 * connection. A typical workflow could be to:
 *
...
rustls_crypto_provider_builder_new_with_base(
  const struct rustls_crypto_provider *base);

/**
 * Customize the supported ciphersuites of the
 * `rustls_crypto_provider_builder`.
 *
 * Returns an error if the builder has already been built. Overwrites any
 * previously set ciphersuites.
 */
rustls_result rustls_crypto_provider_builder_set_cipher_suites(
  struct rustls_crypto_provider_builder *builder,
  const struct rustls_supported_ciphersuite *const *cipher_suites,
  size_t cipher_suites_len);

/**
 * Builds a `rustls_crypto_provider` from the builder and returns it. Returns
 * an error if the builder has already been built.
 *
 * The `rustls_crypto_provider_builder` builder is consumed and should not be
 * used for further calls, except to `rustls_crypto_provider_builder_free`. The
 * caller must still free the builder after a successful build.
 */
rustls_result rustls_crypto_provider_builder_build(
  struct rustls_crypto_provider_builder *builder,
 ...
 *
 * Afterward, the default provider can be retrieved using
 * `rustls_crypto_provider_default`.
 *
 * This can only be done once per process, and will return an error if a
 * default provider has already been set, or if the builder has already been
 * built.
 *
 * The `rustls_crypto_provider_builder` builder is consumed and should not be
 * used for further calls, except to `rustls_crypto_provider_builder_free`. The
 * caller must still free the builder after a successful build.
 */
rustls_result rustls_crypto_provider_builder_build_as_default(
  struct rustls_crypto_provider_builder *builder);
 ...
 * Must not be called twice with the same value.
 */
void rustls_crypto_provider_builder_free(
  struct rustls_crypto_provider_builder *builder);

#if defined(DEFINE_RING)
/**
 * Return the `rustls_crypto_provider` backed by the `*ring*` cryptography
 * library.
 *
...
/**
 * Return the `rustls_crypto_provider` backed by the `aws-lc-rs` cryptography
 * library.
 *
 * The caller owns the returned `rustls_crypto_provider` and must free it using
 * `rustls_crypto_provider_free`.
 ...
 *
 * Using this function expresses in your code that you require FIPS-approved
 * cryptography, and will not compile if you make a mistake with cargo
 * features.
 *
 ...
 *
 * Caller owns the returned `rustls_crypto_provider` and must free it w/
 * `rustls_crypto_provider_free`.
 */
 ...
 */
size_t rustls_crypto_provider_ciphersuites_len(
  const struct rustls_crypto_provider *provider);

/**
 * Retrieve a pointer to a supported ciphersuite of the
 * `rustls_crypto_provider`.
 *
 * This function will return NULL if the `provider` is NULL, or if the index is
 * out of bounds with respect to `rustls_crypto_provider_ciphersuites_len`.
 *
 * The lifetime of the returned `rustls_supported_ciphersuite` is equal to the
 * lifetime of the `provider` and should not be used after the `provider` is
 * freed.
 */
const struct rustls_supported_ciphersuite *
rustls_crypto_provider_ciphersuites_get(
  const struct rustls_crypto_provider *provider, size_t index);
 ...
 * SEC1 formats.
 *
 * When this function returns `rustls_result::Ok` a pointer to a
 * `rustls_signing_key` is written to `signing_key_out`. The caller owns the
 * returned `rustls_signing_key` and must free it with
 * `rustls_signing_key_free`.
 */
rustls_result rustls_crypto_provider_load_key(
  const struct rustls_crypto_provider *provider, const uint8_t *private_key,
  size_t private_key_len, struct rustls_signing_key **signing_key_out);

/**
 * Write `len` bytes of cryptographically secure random data to `buff` using
 * the crypto provider.
 *
 * `buff` must point to a buffer of at least `len` bytes. The caller maintains
 * ownership of the buffer.
 *
 * Returns `RUSTLS_RESULT_OK` on success, or `RUSTLS_RESULT_GET_RANDOM_FAILED`
 * on failure.
 */
rustls_result rustls_crypto_provider_random(
  const struct rustls_crypto_provider *provider, uint8_t *buff, size_t len);
 ...
 * into account.
 */
bool rustls_crypto_provider_fips(
  const struct rustls_crypto_provider *provider);
 ...
 * Must not be called twice with the same value.
 */
void rustls_crypto_provider_free(
  const struct rustls_crypto_provider *provider);

/**
 * Returns the number of ciphersuites the default process-wide crypto provider
 * supports.
 *
 * You can use this to know the maximum allowed index for use with
 * `rustls_default_crypto_provider_ciphersuites_get`.
 *
 * This function will return 0 if no process-wide default
 * `rustls_crypto_provider` is available.
 */
size_t rustls_default_crypto_provider_ciphersuites_len(void);

/**
 * Retrieve a pointer to a supported ciphersuite of the default process-wide
 * crypto provider.
 *
 * This function will return NULL if the `provider` is NULL, or if the index is
 ...
 *
 * The lifetime of the returned `rustls_supported_ciphersuite` is static, as
 * the process-wide default provider lives for as long as the process.
 */
const struct rustls_supported_ciphersuite *
rustls_default_crypto_provider_ciphersuites_get(size_t index);

/**
 * Write `len` bytes of cryptographically secure random data to `buff` using
 * the process-wide default crypto provider.
 *
 * `buff` must point to a buffer of at least `len` bytes. The caller maintains
 * ownership of the buffer.
 *
 * Returns `RUSTLS_RESULT_OK` on success, and one of
 * `RUSTLS_RESULT_NO_DEFAULT_CRYPTO_PROVIDER` or
 * `RUSTLS_RESULT_GET_RANDOM_FAILED` on failure.
 */
rustls_result rustls_default_crypto_provider_random(uint8_t *buff, size_t len);

/**
 * Frees the `rustls_signing_key`. This is safe to call with a `NULL` argument,
 * but must not be called twice with the same value.
 */
void rustls_signing_key_free(struct rustls_signing_key *signing_key);

/**
 * Returns a pointer to the supported `rustls_hpke` Hybrid Public Key
 * Encryption (HPKE) suites, or `NULL` if HPKE is not supported.
 *
 * HPKE is only supported with the `aws-lc-rs` cryptography provider.
 *
 * The returned pointer has a static lifetime equal to that of the program and
 * does not need to be freed.
 */
const struct rustls_hpke *rustls_supported_hpke(void);

/**
 * Convert a `rustls_handshake_kind` to a string with a friendly description of
 * the kind of handshake.
 *
 * The returned `rustls_str` has a static lifetime equal to that of the program
 * and does not need to be manually freed.
 */
 ...
 * returns 0.
 */
size_t rustls_slice_slice_bytes_len(
  const struct rustls_slice_slice_bytes *input);
 ...
 */
struct rustls_slice_bytes rustls_slice_slice_bytes_get(
  const struct rustls_slice_slice_bytes *input, size_t n);
...
/**
 * Create a rustls_server_config_builder using the process default crypto
 * provider.
 *
 * Caller owns the memory and must eventually call
 * rustls_server_config_builder_build, then free the resulting
 * rustls_server_config.
 *
 ...
 * `rustls_server_config_builder_free` to free the builder directly.
 *
 * `tls_versions` set the TLS protocol versions to use when negotiating a TLS
 * session.
 *
 ...
 * `rustls_crypto_provider_builder_set_cipher_suites` for more information.
 */
rustls_result rustls_server_config_builder_new_custom(
  const struct rustls_crypto_provider *provider, const uint16_t *tls_versions,
  size_t tls_versions_len, struct rustls_server_config_builder **builder_out);

/**
 * Create a rustls_server_config_builder for TLS sessions that may verify
 * client certificates.
 *
 * This increases the refcount of `verifier` and doesn't take ownership.
 */
void rustls_server_config_builder_set_client_verifier(
  struct rustls_server_config_builder *builder,
 ...
 *
 * For more control over which secrets are logged, or to customize the format,
 * prefer `rustls_server_config_builder_set_key_log`.
 */
rustls_result rustls_server_config_builder_set_key_log_file(
  struct rustls_server_config_builder *builder);
 ...
 * the security of past, present and future sessions.
 *
 * See also `rustls_server_config_builder_set_key_log_file` for a simpler way
 * to log to a file specified by the `SSLKEYLOGFILE` environment variable.
 */
rustls_result rustls_server_config_builder_set_key_log(
  struct rustls_server_config_builder *builder,
  rustls_keylog_log_callback log_cb,
  rustls_keylog_will_log_callback will_log_cb);

/**
 * "Free" a server_config_builder without building it into a
 * rustls_server_config.
 *
 * Normally builders are built into rustls_server_configs via
 * `rustls_server_config_builder_build` and may not be free'd or otherwise used
 * afterwards.
 *
 * Use free only when the building of a config has to be aborted before a
 * config was created.
 */
void rustls_server_config_builder_free(
  struct rustls_server_config_builder *config);
 ...
 */
rustls_result rustls_server_config_builder_set_ignore_client_order(
  struct rustls_server_config_builder *builder, bool ignore);

/**
 * Set the ALPN protocol list to the given protocols.
 *
 * `protocols` must point to a buffer of `rustls_slice_bytes` (built by the
 * caller) with `len` elements. Each element of the buffer must point to a
 * slice of bytes that contains a single ALPN protocol from
 * <https://www.iana.org/assignments/tls-extensiontype-values/tls-extensiontype-values.xhtml#alpn-protocol-ids>.
 *
 * This function makes a copy of the data in `protocols` and does not retain
 * any pointers, so the caller can free the pointed-to memory after calling.
 ...
 */
rustls_result rustls_server_config_builder_set_alpn_protocols(
  struct rustls_server_config_builder *builder,
  const struct rustls_slice_bytes *protocols, size_t len);
 ...
 * configured certified keys and vice versa.
 */
rustls_result rustls_server_config_builder_set_certified_keys(
  struct rustls_server_config_builder *builder,
  const struct rustls_certified_key *const *certified_keys,
  size_t certified_keys_len);

/**
 * Turn a *rustls_server_config_builder (mutable) into a const
 * *rustls_server_config (read-only). The constructed `rustls_server_config`
 * will be written to the `config_out` pointer when this function returns
 * `rustls_result::Ok`.
 *
 * This function may return an error if no process default crypto provider has
 * been set and the builder was constructed using
 * `rustls_server_config_builder_new`, or if no certificate resolver was set.
 */
rustls_result rustls_server_config_builder_build(
  struct rustls_server_config_builder *builder,
  const struct rustls_server_config **config_out);

/**
 * Returns true if a `rustls_connection` created from the
 * `rustls_server_config` will operate in FIPS mode.
 *
 * This is different from `rustls_crypto_provider_fips` which is concerned
 * only with cryptography, whereas this also covers TLS-level configuration
 * that NIST recommends, as well as ECH HPKE suites if applicable.
 ...
 * unchanged.
 *
 * If this returns a non-error, the memory pointed to by `conn_out` is modified
 * to point at a valid rustls_connection
 *
 * The caller now owns the rustls_connection and must call
 * `rustls_connection_free` when done with it.
 */
rustls_result rustls_server_connection_new(
  const struct rustls_server_config *config,
  struct rustls_connection **conn_out);

/**
 * Returns a `rustls_str` reference to the server name sent by the client in a
 * server name indication (SNI) extension.
 *
 * The returned `rustls_str` is valid until the next mutating function call
 * affecting the connection. A mutating function call is one where the first
 * argument has type `struct rustls_connection *` (as opposed to `const struct
 * rustls_connection *`). The caller does not need to free the `rustls_str`.
 *
 * Returns a zero-length `rustls_str` if:
 *
 * - the connection is not a server connection.
 * - the connection is a server connection but the SNI extension in the client
 * hello has not been processed during the handshake yet. Check
 * `rustls_connection_is_handshaking`.
 * - the SNI value contains null bytes.
 */
struct rustls_str rustls_server_connection_get_server_name(
  const struct rustls_connection *conn);
 ...
 * EXPERIMENTAL: this feature of rustls-ffi is likely to change in the future,
 * as the rustls library is re-evaluating their current approach to client
 * hello handling. Installing a client_hello callback will replace any
 * configured certified keys and vice versa. Same holds true for the
 * set_certified_keys variant.
 */
rustls_result rustls_server_config_builder_set_hello_callback(
  struct rustls_server_config_builder *builder,
 ...
 * inspected in the order given and keys first in the list are given
 * preference, all else being equal. However rustls is free to choose whichever
 * it considers to be the best key with its knowledge about security issues and
 * possible future extensions of the protocol.
 *
 * Return RUSTLS_RESULT_OK if a key was selected and RUSTLS_RESULT_NOT_FOUND
 * if none was suitable.
 */
rustls_result rustls_client_hello_select_certified_key(
  const struct rustls_client_hello *hello,
  const struct rustls_certified_key *const *certified_keys,
  size_t certified_keys_len, const struct rustls_certified_key **out_key);
 ...
 */
void rustls_server_config_builder_set_persistence(
  struct rustls_server_config_builder *builder,
 ...
 * `rustls_client_cert_verifier_builder_build`. Calling with NULL is fine. Must
 * not be called twice with the same value.
 */
void rustls_client_cert_verifier_free(
  struct rustls_client_cert_verifier *verifier);

/**
 * Create a `rustls_web_pki_client_cert_verifier_builder` using the
 * process-wide default cryptography provider.
 *
 ...
 *
 * Without further modification the builder will produce a client certificate
 * verifier that will require a client present a client certificate that chains
 * to one of the trust anchors in the provided `rustls_root_cert_store`. The
 * root cert store must not be empty.
 *
 * Revocation checking will not be performed unless
 * `rustls_web_pki_client_cert_verifier_builder_add_crl` is used to add
 * certificate revocation lists (CRLs) to the builder. If CRLs are added,
 * revocation checking will be performed for the entire certificate chain
 * unless
 * `rustls_web_pki_client_cert_verifier_only_check_end_entity_revocation` is
 * used. Unknown revocation status for certificates considered for revocation
 * status will be treated as an error unless
 * `rustls_web_pki_client_cert_verifier_allow_unknown_revocation_status` is
 * used.
 *
...
/**
 * Add one or more certificate revocation lists (CRLs) to the client
 * certificate verifier builder by reading the CRL content from the provided
 * buffer of PEM encoded content.
 *
 * By default revocation checking will be performed on the entire certificate
 * chain. To only check the revocation status of the end entity certificate,
 * use `rustls_web_pki_client_cert_verifier_only_check_end_entity_revocation`.
 *
 * This function returns an error if the provided buffer is not valid PEM
 * encoded content.
 */
rustls_result rustls_web_pki_client_cert_verifier_builder_add_crl(
  struct rustls_web_pki_client_cert_verifier_builder *builder,
  const uint8_t *crl_pem, size_t crl_pem_len);

/**
 * When CRLs are provided with
 * `rustls_web_pki_client_cert_verifier_builder_add_crl`, only check the
 * revocation status of end entity certificates, ignoring any intermediate
...
/**
 * When CRLs are provided with
 * `rustls_web_pki_client_cert_verifier_builder_add_crl`, and it isn't possible
 * to determine the revocation status of a considered certificate, do not treat
 * it as an error condition.
 *
 * Overrides the default behavior where unknown revocation status is considered
 * an error.
 */
rustls_result
...
/**
 * Allow unauthenticated anonymous clients in addition to those that present a
 * client certificate that chains to one of the verifier's configured trust
 * anchors.
 */
rustls_result
 ...
 *
 * By default, the client cert verifier will use the subjects provided by the
 * root cert store configured for client authentication. Calling this function
 * will remove these hint subjects, indicating the client should make a free
 * choice of which certificate to send.
 */
rustls_result rustls_web_pki_client_cert_verifier_clear_root_hint_subjects(
  struct rustls_web_pki_client_cert_verifier_builder *builder);

/**
 * Add additional distinguished names to the list of trust anchor hint
 * subjects.
 *
 * By default, the client cert verifier will use the subjects provided by the
 * root cert store configured for client authentication. Calling this function
 * will add to these existing hint subjects. Calling this function with an
 * empty `store` will have no effect, use
 * `rustls_web_pki_client_cert_verifier_clear_root_hint_subjects` to clear the
 * subject hints.
 */
rustls_result rustls_web_pki_client_cert_verifier_add_root_hint_subjects(
  struct rustls_web_pki_client_cert_verifier_builder *builder,
  const struct rustls_root_cert_store *store);
 ...
 * The builder is consumed and cannot be used again, but must still be freed.
 *
 * The verifier can be used in several `rustls_server_config` instances and
 * must be freed by the application when no longer needed. See the
 * documentation of `rustls_web_pki_client_cert_verifier_builder_free` for
 * details about lifetime.
 */
rustls_result rustls_web_pki_client_cert_verifier_builder_build(
  struct rustls_web_pki_client_cert_verifier_builder *builder,
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 */
void rustls_web_pki_client_cert_verifier_builder_free(
  struct rustls_web_pki_client_cert_verifier_builder *builder);

/**
 * Create a `rustls_web_pki_server_cert_verifier_builder` using the
 * process-wide default crypto provider. Caller owns the memory and may free it
 * with
 *
 * Caller owns the memory and may free it with
 ...
 *
 * Without further modification the builder will produce a server certificate
 * verifier that will require a server present a certificate that chains to one
 * of the trust anchors in the provided `rustls_root_cert_store`. The root cert
 * store must not be empty.
 *
 * Revocation checking will not be performed unless
 * `rustls_web_pki_server_cert_verifier_builder_add_crl` is used to add
 * certificate revocation lists (CRLs) to the builder.  If CRLs are added,
 * revocation checking will be performed for the entire certificate chain
 * unless
 * `rustls_web_pki_server_cert_verifier_only_check_end_entity_revocation` is
 * used. Unknown revocation status for certificates considered for revocation
 * status will be treated as an error unless
 * `rustls_web_pki_server_cert_verifier_allow_unknown_revocation_status` is
 * used.
 *
 * This copies the contents of the `rustls_root_cert_store`. It does not take
 * ownership of the pointed-to data.
...
/**
 * Add one or more certificate revocation lists (CRLs) to the server
 * certificate verifier builder by reading the CRL content from the provided
 * buffer of PEM encoded content.
 *
 * By default revocation checking will be performed on the entire certificate
 * chain. To only check the revocation status of the end entity certificate,
 * use `rustls_web_pki_server_cert_verifier_only_check_end_entity_revocation`.
 *
 * This function returns an error if the provided buffer is not valid PEM
 * encoded content.
 */
rustls_result rustls_web_pki_server_cert_verifier_builder_add_crl(
  struct rustls_web_pki_server_cert_verifier_builder *builder,
  const uint8_t *crl_pem, size_t crl_pem_len);

/**
 * When CRLs are provided with
 * `rustls_web_pki_server_cert_verifier_builder_add_crl`, only check the
 * revocation status of end entity certificates, ignoring any intermediate
 * certificates in the chain.
 */
rustls_result
...
/**
 * When CRLs are provided with
 * `rustls_web_pki_server_cert_verifier_builder_add_crl`, and it isn't possible
 * to determine the revocation status of a considered certificate, do not treat
 * it as an error condition.
 *
 * Overrides the default behavior where unknown revocation status is considered
...
/**
 * When CRLs are provided with
 * `rustls_web_pki_server_cert_verifier_builder_add_crl`, and the CRL
 * nextUpdate field is in the past, treat it as an error condition.
 *
 * Overrides the default behavior where CRL expiration is ignored.
 */
rustls_result rustls_web_pki_server_cert_verifier_enforce_revocation_expiry(
  struct rustls_web_pki_server_cert_verifier_builder *builder);
 ...
 * The builder is consumed and cannot be used again, but must still be freed.
 *
 * The verifier can be used in several `rustls_client_config` instances and
 * must be freed by the application when no longer needed. See the
 * documentation of `rustls_web_pki_server_cert_verifier_builder_free` for
 * details about lifetime.
 */
rustls_result rustls_web_pki_server_cert_verifier_builder_build(
  struct rustls_web_pki_server_cert_verifier_builder *builder,
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 */
void rustls_web_pki_server_cert_verifier_builder_free(
  struct rustls_web_pki_server_cert_verifier_builder *builder);
 ...
 *
 * The verifier can be used in several `rustls_client_config` instances and
 * must be freed by the application using `rustls_server_cert_verifier_free`
 ...
 * `rustls_server_cert_verifier_builder_build` or
 * `rustls_platform_server_cert_verifier`.
 *
 * Calling with NULL is fine. Must not be called twice with the same value.
 */
void rustls_server_cert_verifier_free(
  struct rustls_server_cert_verifier *verifier);
